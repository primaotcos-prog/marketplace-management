...
if (!window.__listingStatusToolsLoaded) {
  ...
  // The UI talks only to the existing Supabase GameBoost adapter.
  // The adapter owns the real GameBoost endpoints:
  // POST /item-offers/{id}/list and POST /item-offers/{id}/unlist,
  // followed by a GET verification. Do not call GameBoost directly here.
    ...
    if(error){
      ...
      try{
        if(error.context?.json){
          ...
          detail=d?.error||d?.message||(d?.gameboost_response?JSON.stringify(d.gameboost_response):'');
        }
      }catch{}
      throw new Error(detail||error.message||'GameBoost API error');
    }
    if(!data?.ok)throw new Error(data?.error||'GameBoost API error');
    return data;
  };
  ...
  const load=async()=>{
    const r=await supabase
      .from('listings')
      .select('id,title,external_offer_id,status,stock,price,currency,metadata')
  ...
  const open=async(pre=[])=>{
    ...
    try{
      ...
      if(!items.length){
        list.innerHTML='<div class="empty">Tidak ada listing.</div>';
        count.textContent='0 listing';
        return;
      }

      list.innerHTML=items.map(x=>`<label class="lst-item"><input type="checkbox" data-id="${esc(x.id)}"${selected.has(String(x.id))?' checked':''}><span><strong>${esc(x.title||'Untitled')}</strong><small>Offer ID: ${esc(x.external_offer_id||'-')} · Stock ${Number(x.stock||0)} · ${esc(x.price??'-')} ${esc(x.currency||'')}</small></span><span class="lst-badge ${isOn(x)?'':'off'}">${isOn(x)?'ON':'OFF'}</span></label>`).join('');

      const update=()=>count.textContent=`${list.querySelectorAll('[data-id]:checked').length} dipilih dari ${items.length}`;
      list.addEventListener('change',update);
      update();

      w.querySelector('[data-all]').onclick=()=>{
        list.querySelectorAll('[data-id]').forEach(x=>x.checked=true);
        update();
      };
      w.querySelector('[data-none]').onclick=()=>{
        list.querySelectorAll('[data-id]').forEach(x=>x.checked=false);
        update();
      };
      ...
      w.querySelector('[data-apply]').onclick=async()=>{
        ...
        if(!ids.length){
          msg.className='lst-msg err';
          msg.textContent='Pilih minimal 1 listing.';
          return;
        }
        ...
        if(!pending.length){
          msg.className='lst-msg ok';
          msg.textContent=`Semua ${targets.length} listing sudah ${action==='list'?'ON':'OFF'}. Tidak ada request GameBoost yang perlu dikirim.`;
          return;
        }
        ...
        for(const x of pending){
          try{
            ...
            // IMPORTANT: this is the real adapter operation. It maps to
            // POST /item-offers/{id}/list or /unlist and verifies the result.
            const result=await api({
              operation:'set_offer_status',
              offer_id:x.external_offer_id,
              action
            });
            ...
            if(remoteStatus && !expected.includes(remoteStatus)){
              throw Error(`Status GameBoost tidak sesuai setelah perubahan: ${remoteStatus}`);
            }
            ok++;
          }catch(e){
            fail++;
            errors.push(`${x.title||x.id}: ${e.message}`);
          }
        }
        ...
        // The adapter already persists the verified remote offer locally.
        // Refresh the listing table so the UI never invents a status.
        if(ok||fail)document.querySelector('[data-sync]')?.click();
      };
    }catch(e){
      list.innerHTML=`<div class="empty">Gagal memuat listing: ${esc(e.message)}</div>`;
      count.textContent='Gagal';
    }
  };
  ...
  const inject=()=>{
    ...
    if(!head.querySelector('[data-status-manager]')){
      ...
      b.type='button';
      b.className='action listing-tool-status';
      b.dataset.statusManager='1';
      b.textContent='ON / OFF Listings';
      b.onclick=()=>open();
  ...
  // Support the row-level ON/OFF buttons already rendered by app.js.
  // We intercept only those buttons and route them through the same adapter.
