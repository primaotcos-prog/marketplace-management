...
async function verifySignature(request, rawBody, secret) {
  const supplied = text(first(request.headers.get("x-gameboost-signature"), request.headers.get("Signature"), request.headers.get("x-webhook-signature"))).replace(/^sha256=/i, "");
  if (!supplied) return false;
  return safeEqual(supplied.toLowerCase(), (await signature(secret, rawBody)).toLowerCase());
}
...
function normalizeEvent(request, envelope) {
  const eventType = text(first(request.headers.get("x-gameboost-topic"), envelope?.event, envelope?.event_type, envelope?.type, "unknown"));
  ...
  return {
    eventType,
    eventId: text(first(request.headers.get("x-gameboost-event-id"), envelope?.event_id, envelope?.id)),
...
function isAuthorizedTestRequest(request, url, env) {
  const supplied = text(first(request.headers.get("x-gameboost-test-token"), url.searchParams.get("token")));
...
async function runTemporaryWebhookTest(env) {
  ...
  const envelope = { event: "item.order.purchased", event_id: testOrderId, test_mode: true, payload: { id: testOrderId, item_offer_id: "WEBHOOK_TEST_OFFER", title: "Temporary Webhook Test", quantity: 1, status: "pending", price_eur: "0.01", currency: "EUR", buyer: { username: "webhook-test" }, created_at: new Date().toISOString() } };
  const rawBody = JSON.stringify(envelope);
  const headers = new Headers({ "content-type": "application/json", "x-gameboost-event-id": testOrderId, "x-gameboost-topic": "item.order.purchased", "x-gameboost-signature": await signature(secret, rawBody), "x-gameboost-test-mode": "1" });
  const response = await onRequestPost({ request: new Request("https://temporary-test.local/webhooks/gameboost", { method: "POST", headers, body: rawBody }), env });
...
export async function onRequestGet({ request, env }) {
  ...
  return json({ ok: true, service: "gameboost-webhook", configured: Boolean(text(env.GAMEBOOST_WEBHOOK_SECRET) && text(env.SUPABASE_URL) && text(env.SUPABASE_SERVICE_ROLE_KEY)), endpoint: "/webhooks/gameboost", test_endpoint: "/webhooks/gameboost/test?token=YOUR_TEST_TOKEN" });
}
export async function onRequestPost({ request, env }) {
  ...
  try {
    ...
    if (!existing?.[0]) await supabaseRequest(env, "webhook_events", { method: "POST", body: JSON.stringify({ marketplace: "gameboost", event_id: eventId, event_type: info.eventType, payload: envelope, status: "received" }) });
    ...
    if (!info.orderId) {
      await supabaseRequest(env, `webhook_events?marketplace=eq.gameboost&event_id=eq.${escPath(eventId)}`, { method: "PATCH", body: JSON.stringify({ status: "processed", processed_at: new Date().toISOString() }) });
    ...
    const orderRow = { user_id: account.user_id, marketplace_account_id: account.id, product_id: listing?.product_id || null, external_order_id: info.orderId, buyer_reference: info.buyerReference || null, quantity: info.quantity, amount: info.amount, currency: info.currency, status: mapped.status, delivery_status: mapped.delivery_status, raw_data: envelope, updated_at: new Date().toISOString() };
    ...
    let savedOrder;
    if (old?.[0]) { const updated = await supabaseRequest(env, `orders?id=eq.${escPath(old[0].id)}`, { method: "PATCH", body: JSON.stringify(orderRow) }); savedOrder = updated?.[0] || old[0]; }
    else { const created = await supabaseRequest(env, "orders", { method: "POST", body: JSON.stringify(orderRow) }); savedOrder = created?.[0] || null; }
    if (!isTestMode && listing && info.eventType === "item.order.purchased") {
      const nextStock = Math.max(0, (num(listing.stock) ?? 0) - info.quantity);
      await supabaseRequest(env, `listings?id=eq.${escPath(listing.id)}`, { method: "PATCH", body: JSON.stringify({ stock: nextStock, status: nextStock === 0 ? "sold_out" : listing.status, updated_at: new Date().toISOString() }) });
    }
    await supabaseRequest(env, `webhook_events?marketplace=eq.gameboost&event_id=eq.${escPath(eventId)}`, { method: "PATCH", body: JSON.stringify({ status: "processed", processed_at: new Date().toISOString(), error_message: null }) });
    await supabaseRequest(env, "sync_logs", { method: "POST", body: JSON.stringify({ user_id: account.user_id, marketplace_account_id: account.id, operation: isTestMode ? "gameboost_webhook_test" : "gameboost_webhook", status: "success", message: `${info.eventType} received for order ${info.orderId}${isTestMode ? " (TEST MODE)" : ""}.`, metadata: { event_id: eventId, event_type: info.eventType, offer_id: info.offerId || null, order_id: savedOrder?.id || null, listing_matched: Boolean(listing), test_mode: isTestMode } }) });
    ...
    try { await supabaseRequest(env, `webhook_events?marketplace=eq.gameboost&event_id=eq.${escPath(eventId)}`, { method: "PATCH", body: JSON.stringify({ status: "failed", error_message: message }) }); } catch {}
    return json({ ok: false, event_id: eventId, error: message, test_mode: isTestMode }, 500);
  }
}
