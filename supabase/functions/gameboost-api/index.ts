...
if(op==='import_offers'){const started=Date.now();try{return json({...await importOffers(admin,user),duration_ms:Date.now()-started})}catch(e){return json({ok:false,error:e instanceof Error?e.message:String(e),duration_ms:Date.now()-started},502)}}
if(op==='list_games'){const q=String(body?.search||'').trim();const params=new URLSearchParams();if(q)params.set('filter[search]',q);params.set('sort',String(body?.sort||'name'));const gb=await gbRequest(`/games?${params.toString()}`);return json({ok:true,gameboost_status:gb.status,games:offers(gb.data),message:gb.data?.message??null})}
...
}catch(e){const s=(e as any)?.status;return json({ok:false,error:e instanceof Error?e.message:String(e)},s===401?401:500)}});